...
do
	...
	local imgui = guiParent:FindFirstChild("imgui")
...
local function connectLibrary(signal, callback)
    local connection = signal:Connect(callback)
    table.insert(libraryConnections, connection)
    return connection
end

connectLibrary(UIS.InputBegan, function(input, gameProcessed)
...
function library:AddWindow(title, options)
	...
	do -- Altering Window Color
        ...
        if not (Window and Window.Parent) then
            break -- window destroyed, stop the loop
        end
	...
	do -- Resize Window
		...
		local Held = false
		connect(UIS.InputBegan, function(inputObject)
		...
		connect(UIS.InputEnded, function(inputObject)
			if inputObject.UserInputType == Enum.UserInputType.MouseButton1 then
				Held = false
			end
		end)
				...
				do -- Tab Elements
					...
					function tab_data:AddTextBox(textbox_text, callback, textbox_options)
						...
						textbox.FocusLost:Connect(function(enterPressed)
						    ...
						    if not success then
						        warn("AddTextBox callback error:", err)
						    end
						end)
									...
									spawn(function() -- Loop check
										if Entered and not slider_options.readonly then
											while Held and (not dropdown_open) do
												...
												indicator.Size = UDim2.new(size or min, 0, 0, 20)
					...
					function tab_data:AddKeybind(keybind_name, callback, keybind_options)
						...
						connect(UIS.InputBegan, function(a, b)
...
local object = prefabs:FindFirstChild("DropdownButton"):Clone()
					...
					function tab_data:AddConsole(console_options)
						...
						local sf = console:FindFirstChildWhichIsA("ScrollingFrame")
						...
						sf.MouseEnter:Connect(function()
						    new_tab.ScrollingEnabled = false
						end)
						...
						if console_options.readonly then
						    Source.Active = false
						    Source.TextEditable = false
						end

						for _, child in pairs(Source:GetChildren()) do
							child.ZIndex = child.ZIndex + (windows * 10) + 1
						end

						if Source:FindFirstChild("Comments") then
						...
						local function update_line_numbers(text)
						    local line_count = 1
						    ...
						    local numbers = table.create(line_count)
						
						    for i = 1, line_count do
						        numbers[i] = tostring(i)
						    end
						
						    Lines.Text = table.concat(numbers, "\n") .. "\n"
						end

						local consoleUpdateScheduled = false
						local consoleUpdateDirty = false
						
						local function is_near_bottom()
						    local bottom = math.max(
						        0,
						        sf.AbsoluteCanvasSize.Y - sf.AbsoluteSize.Y
						    )
						
						    return sf.CanvasPosition.Y >= bottom - 30
						end
						
						local function update_console_size()
						    if consoleUpdateScheduled then
						        consoleUpdateDirty = true
						        return
						    end
						    ...
						    task.spawn(function()
						        repeat
						            ...
						            local shouldAutoScroll = is_near_bottom()
						
									local availableWidth = math.max(
									    1,
									    sf.AbsoluteSize.X - 40
									)
						            ...
						            if shouldAutoScroll then
						                sf.CanvasPosition = Vector2.new(
						                    ...
						                    math.max(
						                        0,
						                        contentHeight - sf.AbsoluteSize.Y
						                    )
						                )
						            end
						
						        until not consoleUpdateDirty
						
						        consoleUpdateScheduled = false
						    end)
						end

						local function set_log_text()
						    local text
						
						    if #log_lines == 0 then
						        text = ""
						    else
						        text = table.concat(log_lines, "\n") .. "\n"
						    end
						
						    Source.Text = text
						
						    if Lines.Visible then
						        update_line_numbers(text)
						    end
						...
						if console_options.source == "Lua" then
							...
							local function comments(text)
								...
								while i <= #text do
									local c = text:sub(i, i)
									if not in_comment and c == "-" and text:sub(i, i + 1) == "--" then
										in_comment = true
										out[i] = "-"
										i = i + 1
										out[i] = "-"
							...
							local function highlight_lua()
								...
								if Source:FindFirstChild("RemoteHighlight") then Source.RemoteHighlight.Text = Highlight(s, {"FireServer", "fireServer", "InvokeServer", "invokeServer"}) end
								if Source:FindFirstChild("Tokens") then Source.Tokens.Text = hTokens(s) end
								if Source:FindFirstChild("Numbers") then Source.Numbers.Text = numbers(s) end
								if Source:FindFirstChild("Strings") then Source.Strings.Text = strings(s) end
								if Source:FindFirstChild("Comments") then Source.Comments.Text = comments(s) end

								update_line_numbers(s)
							end
						...
						function console_data:Set(code)
							...
							if console_options.source == "Logs" then
								table.clear(log_lines)
					...
					function tab_data:AddFolder(folder_name) -- [Folder]
						...
						button.MouseButton1Click:Connect(function()
							if open then -- Close
								...
								Resize(folder, {Size = UDim2.new(1, 0, 0, 20)}, options.tween_time)
							else -- Open
								Resize(toggle, {Rotation = 90}, options.tween_time)
								objects.Visible = true
								Resize(folder, {Size = UDim2.new(1, 0, 0, gFolderLen())}, options.tween_time)
							end
